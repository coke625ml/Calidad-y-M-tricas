...
-- select top 10 * from gdc..tbl_llamadas_COMDATA_inicial
...
drop table #tab1
select a.*,
	   SUBSTRING(Datos_ingresados,1,3) RUC_Codigo,
       case when SUBSTRING(Datos_ingresados,1,3)='RUC'
				 then cast(SUBSTRING(Datos_ingresados,6,11) as float)
       else NULL end as RUC, 
       
	   case when SUBSTRING(Datos_ingresados,1,3)='COD'
				 then cast(SUBSTRING(Datos_ingresados,18,9) as float)
...
insert into gdc..Llamadas_In_Comdata
select *
from #tab8

-- 8. actualizar en la tabla inicial de llamadas los nuevos registros para que en el campo: FLAG_PROCESO, aparezcan como USED

update gdc..tbl_llamadas_COMDATA_inicial
set Flag_Proceso = 'Used'
where Flag_Proceso = 'New'



-- select count(*) as q from gdc..Llamadas_In_Comdata
-- select * from gdc..Llamadas_In_Comdata
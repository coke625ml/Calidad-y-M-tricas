...
as 
select
	isnull(c.segmento_nuevo,'Independiente') as Nuevo_Segmento,
	isnull(c.subsegmento_nuevo,'Independiente') as Nuevo_Subsegmento,
	...
	d.*
from 
	(select periodo as periodo_s, ruc, codigo as codigo_s, fechaalta, mob, moi, movimiento, estado, vol, txn, comisionvisanet, mccnac, agrupaciongiros, producto, subproducto, comerciostemporales, LimaProvincias 
...
left join (select ruc as ruc_seg, periodo as periodo_seg, segmento as segmento_nuevo, subSegmento as subsegmento_nuevo from [Visanet Planeamiento].[DBO].[V_Segmento_Cliente]) as c on b.ruc = c.ruc_seg and b.periodo_s = c.periodo_seg
left join (select * from [Visanet Planeamiento].[Comercial].[Desafiliacion] where Periodo >= 201712) as e on b.codigo_s = e.Codigo and b.periodo_s = e.Periodo
left join (select * from gdc..Afiliacionesproa where alta >= 201712) as m on b.codigo_s = m.codigo
GO
